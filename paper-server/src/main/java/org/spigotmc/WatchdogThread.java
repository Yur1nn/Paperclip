...
public class WatchdogThread extends ca.spottedleaf.moonrise.common.util.TickThread { // Paper - rewrite chunk system
    ...
    private WatchdogThread(long timeoutTime, boolean restart) {
        super("Paperclip Watchdog Thread");
        ...
        while (!this.stopping) {
            ...
            if (this.lastTick != 0 && this.timeoutTime > 0 && WatchdogThread.hasStarted && (!server.isRunning() || (currentTime > this.lastTick + this.earlyWarningEvery && !DISABLE_WATCHDOG))) { // Paper - add property to disable
                ...
                if (isLongTimeout) {
                    ...
                    logger.log(Level.SEVERE, "The server has stopped responding! This is (probably) not a Paperclip bug."); // Paper
                    ...
                    logger.log(Level.SEVERE, "If you are unsure or still think this is a Paperclip bug, please report this to your Paperclip fork issue tracker.");
                    logger.log(Level.SEVERE, "Be sure to include ALL relevant console errors and Minecraft crash reports");
                    logger.log(Level.SEVERE, "Paperclip version: " + Bukkit.getServer().getVersion());
                    ...
                    logger.log(Level.SEVERE, "--- DO NOT REPORT THIS TO PAPERCLIP - THIS IS NOT A BUG OR A CRASH  - " + Bukkit.getServer().getVersion() + " ---");
                    logger.log(Level.SEVERE, "The server has not responded for " + (currentTime - lastTick) / 1000 + " seconds! Creating thread dump");
                }
                // Paper end - Different message for short timeout
                logger.log(Level.SEVERE, "------------------------------");
                logger.log(Level.SEVERE, "Server thread dump (Look for plugins here before reporting to Paperclip!):"); // Paper
                ...
                if (isLongTimeout) {
                    ...
                    logger.log(Level.SEVERE, "--- DO NOT REPORT THIS TO PAPERCLIP - THIS IS NOT A BUG OR A CRASH ---");
                }
